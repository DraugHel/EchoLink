...
## Chat-History-Suche / überprüfbare frühere Chats (2026-09-06)

- `server/lib/chatHistorySearch.js` — strikte Query-/Filtervalidierung, user-gebundene FTS5-Suche und begrenztes Nachlesen tatsächlicher Nachrichtenreihenfolge; kein LIKE-Fallback und keine Anhangsextraktion.
- `server/lib/chatHistoryTools.js` — interaktive Tools `search_chat_history` und `read_chat_excerpt`, 3+3 Aufrufgrenzen, requestgebundene Deduplizierung, Laufzeit-/Zeichenbudget und Quellenkatalog.
- `server/lib/chatHistoryEvidence.js` — persistiert nur tatsächlich zitierte `[H…]`-Referenzen und löst sie beim Reload gegen die aktuelle eigene Quelle auf.
- `server/lib/memoryRecallPolicy.js` / `server/routes/chat.js` — Recall-only erlaubt ausschließlich History-Tools; Memory-Inventur bleibt toolfrei, normale aktuelle Arbeit behält alle bisherigen Tools plus History. Serverseitige Ausführung prüft die Request-Allowlist vor jedem Side Effect.
- `server/routes/conversations.js` — History-Belege werden beim Nachrichten-Reload live auf `available`, `changed` oder `unavailable` aufgelöst.
- `client/src/components/ChatHistorySources.jsx` — kleine Quellenliste „Frühere Chats“ mit Navigation über den bestehenden `openSearchResult()`-/Scrollpfad, einschließlich archivierter Chats.
- `docs/chat-history-search.md` — Verträge, Grenzen, Sicherheit, Quellen- und Betriebsverhalten.
